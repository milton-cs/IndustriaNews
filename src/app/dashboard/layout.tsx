...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("full_name, role, advertiser_id")
    .eq("id", user.id)
    .single()
  ...
  return (
    <div className="min-h-screen bg-brand-grafite text-white">
      <header className="border-b border-white/10 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <form action="/api/auth/signout" method="POST">
            <button className="text-xs text-gray-500 hover:text-white transition-colors font-headline uppercase">
              Sair
            </button>
          </form>
        </div>
      ...
      <div className="flex">
        <DashboardNav role={profile?.role || "anunciante"} />
